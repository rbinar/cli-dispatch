...
import { rmSync, readFileSync, writeFileSync, existsSync, mkdtempSync } from 'node:fs'
...
const parserScript = path.join(SELF_DIR, '..', 'cp-stream-parse.mjs')
// Use the OS temp dir (not a repo-relative path) so a stray run never leaves
// a junk directory behind in the working tree.
const testDir = mkdtempSync(path.join(os.tmpdir(), 'cli-dispatch-test-cp-session-'))
