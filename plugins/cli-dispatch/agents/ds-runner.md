...
- **Preferred primitive when you must block on an already-running background session:** `cli-dispatch-wait <session-id> [--timeout SECS]` blocks at zero LLM-token cost (a plain shell poll loop, not an LLM turn) until the session reaches a terminal state (`done`/`error`/`killed`), then prints a compact status summary — use it INSTEAD of a hand-rolled `sleep N && cat status.json` loop whenever you need to wait on a session that is already running in the background (i.e. NOT the case where `ds-agent` itself is already blocking you in the foreground — that case needs no change). This does not relax the terminal-state gate or the never-fire-and-forget rule below — it is a cheaper way to satisfy them, not a replacement for them.
- If you must poll `status.json` (e.g. after reattaching), sleep at least 30-60s between checks, bounded iterations; every extra turn re-reads your entire growing context and that cache-read cost dominated real-world runs (~297M tokens in one measured session). (waiting stays synchronous and inline per the rules above)
