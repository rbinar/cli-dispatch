...
use cp-worktree-run.sh — run cp-agent --cwd <repo> directly, even for a repo-changing
...
cp-worktree-run.sh errors, retry AT MOST once with a new branch name, then report the
