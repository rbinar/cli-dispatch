---
description: List claude-ds (DeepSeek) worker sessions
allowed-tools: Bash
---

# DeepSeek sessions

List delegations started with `claude-ds-stream` (DeepSeek), newest first. All backends share
one session root; this view filters to `backend: deepseek` only. For every backend at once use
...
```bash
CLI_DISPATCH_BACKEND_FILTER=deepseek node <<'EOF'
