...
List delegations started with `cp-stream` (GitHub Copilot), newest first. All backends share one
session root; this view filters to `backend: copilot` only. For every backend at once use
...
```bash
CLI_DISPATCH_BACKEND_FILTER=copilot node <<'EOF'
...
if (!fs.existsSync(root)) { console.log('(no sessions yet — start one with /cli-dispatch:cp-run)'); process.exit(0) }
