---
description: List OpenCode (OpenRouter) worker sessions
allowed-tools: Bash
---

# OpenCode sessions

List delegations started with `oc-stream` (OpenCode), newest first. All backends share one
session root; this view filters to `backend: opencode` only. For every backend at once use
...
```bash
CLI_DISPATCH_BACKEND_FILTER=opencode node <<'EOF'
...
if (!fs.existsSync(root)) { console.log('(no sessions yet — start one with /cli-dispatch:oc-run)'); process.exit(0) }
